...
# API Testing Helper Script (POST)
# Usage: ./scripts/api-post.sh /path/to/endpoint '{"key": "value"}'
#
...
if [[ -z "${1:-}" ]]; then
    echo "Usage: $0 /path/to/endpoint [json-body]" >&2
    echo "Example: $0 /rule-groups/batch '{\"ids\": [\"uuid-1\", \"uuid-2\"]}'" >&2
    exit 1
fi
...
AUTH_RESULT=$(AWS_PROFILE=$PROFILE AWS_REGION=$REGION aws cognito-idp initiate-auth \
...
echo "Requesting: POST $URL" >&2
if [[ -n "$JSON_BODY" ]]; then
    echo "Body: $JSON_BODY" >&2
fi
echo "---" >&2
