 ...
 *
 * HP / CON note: the legacy engine captured the CON modifier at level-up time, so a CON raised
 * later did not retroactively grow this level's HP — and in every scenario CON is
 * set after the class levels, so that captured value was 0. Derives are pure
 * over live facts (no level-up capture), so the CON modifier is counted once (by
 * level 1's `10 + con.modifier`) and level 2 adds a flat +6. This reproduces the
