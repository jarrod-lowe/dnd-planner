...
/** Skill → governing ability (the v1 group's 18 skills; 5e 2024). */
...
function increaseOffer(a: Ability): Offer {
  ...
  return {
    ...
    apply: (f) => {
      const diagnostics: Diagnostic[] = [];
      // v1 increments then flags > 20, i.e. an increase from >= 20 is illegal.
...
function skillProficiencyOffer(skill: string): Offer {
  ...
  return {
    ...
    ui: {
      section: 'configuration',
      name: `${AS}.${id}.name`,
      // proficiency-acrobatics is the layout anchor in v1; the rest pack behind it.
 ...
 *
 * The functional shape vs the 3.7k-line v1 group:
