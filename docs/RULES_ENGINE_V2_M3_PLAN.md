...
- **Wave D — weapons (retire the Python preprocessor):** `dagger`(+`-mastery`),
  ...
  `spear`, `spear-plus1`. The v1 weapons are generated by
  `scripts/rule_preprocessor` from `data/rule-sources`; re-author as modules (or
  retarget the generator to emit modules) and retire the YAML generation.
