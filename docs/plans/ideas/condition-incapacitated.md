...
- **Composition contract (THIS DOC DEFINES IT):** wave-5 children (Paralyzed/Petrified/Stunned) and Unconscious write `condition.incapacitated: 1` DIRECTLY in their own effect state (`stateCombine: 'max'` on that fact — multi-writer safe), with NO cross-module import or shared helper — the shared fact name IS the coupling. Readers gate `> 0`, never `=== 1`. condition-incapacitated owns ONLY the standalone recorder/effect/notice; children depend on nothing in it. A child's own eviction/clear removes its incapacitated contribution automatically (it lives in the child's effect state).
