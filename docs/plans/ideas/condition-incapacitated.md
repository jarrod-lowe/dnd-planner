...
- **REVERSE ordering — accepted, NOT enforced (illegal-but-visible):** condition already standing → player plans cast-bless anyway → the row renders ILLEGAL (action denial diagnostic) but its apply still advertises the hold, and endTurn commits it — the character ends the turn incapacitated AND concentrating. That is the engine's DOCUMENTED contract (plan.ts: "the action still applies when illegal"; "illegal but still executes, like every other illegal planned row") — the same class as a planned-anyway dash committing its boost while halted. REJECTED: making holds react to `condition.incapacitated` (or suppressing the advertise in the cast apply) — that converts illegal into IMPOSSIBLE, breaking the contract (apply re-checks ADD diagnostics, never withdraw) and policing concentration uniquely (every other illegal row commits). The contradiction stays VISIBLE: the standing notice already says Concentration is broken. PIN the edge with a scenario so the behavior is asserted, not accidental.
...
- `incapacitated-then-cast-planned-anyway` (acceptance pin, the dash-mask shape): condition first → cast bless (row ILLEGAL, no-action diagnostic present) → endTurn → hold committed alongside the condition — illegal-but-visible by design; NOT fixed by suppression
