...
- [ ] RED: `condition-deafened-record` scenario fails — right reason: unknown group → skipped vs `EXPECTED_RUNNABLE`
- [ ] `condition-deafened.ts`: `record-deafened` offer, keyed deafened effect, notice annotate
...
- [ ] terraform seed `char_condition_deafened_rulegroup_seed` (dynamodb-items.tf); `make validate` passes
