...
describe('ruleGroupCache', () => {
  ...
  describe('ensureCached', () => {
    it('returns cached data without making an API call', async () => {
      seedCache({
        'rg-1': { name: 'Fireball', description: 'A fire spell' }
      });

      const result = await ensureCached(['rg-1'], 'en');

      expect(mockApiPost).not.toHaveBeenCalled();
      expect(result.get('rg-1')).toEqual({ name: 'Fireball', description: 'A fire spell' });
    });
    ...
    it('fetches uncached IDs via batch API and merges into cache', async () => {
      ...
      mockApiPost.mockResolvedValue({
        ok: true,
        json: async () => ({
          ruleGroups: [{ ruleGroupId: 'rg-2', name: 'Shield', description: 'A shield spell' }]
        })
      } as Response);
      ...
      expect(result.get('rg-1')).toEqual({ name: 'Fireball', description: 'A fire spell' });
      expect(result.get('rg-2')).toEqual({ name: 'Shield', description: 'A shield spell' });
      ...
      expect(cachedResult.get('rg-2')).toEqual({ name: 'Shield', description: 'A shield spell' });
    });
