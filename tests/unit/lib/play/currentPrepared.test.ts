...
describe('currentPrepared', () => {
   ...
   * character never had.
   *
   * The edge is unreachable in practice today: the one always-prepared grant
   * (Oath of Redemption) writes `prepared` via `max` alongside
   * `alwaysPrepared`, so real always-prepared spells ARE seeded — by their
   * prepared fact, like every other spell. This test pins the contract should a
   * future grant write only `alwaysPrepared`.
   */
  ...
  it('seeds an always-prepared spell the grant also wrote prepared, like any other', () => {
    // The Oath of Redemption pattern: the grant writes BOTH facts.
    const facts: Facts = {
      [sleep.preparedFact]: 1,
      [sleep.alwaysPreparedFact]: 1
    };

    expect(currentPrepared(modules, facts)).toEqual([sleep]);
  });
});
