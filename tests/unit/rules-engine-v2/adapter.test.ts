...
import { plannedEntry, plannedEntries } from '$lib/rules-engine-v2/adapter';
import type { EngineOutput, PlannedRef } from '$lib/rules-engine-v2';

/**
 * M4/W1 contract adapter — folding per-instance `planDiagnostics` back onto planned
 * items (what v1 carried inside each planned rule's own entry).
 */
