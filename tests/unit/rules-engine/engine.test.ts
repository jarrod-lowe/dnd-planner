...
import type { EngineInput, PlannedRef, RuleModule } from '$lib/rules-engine';
 ...
 * output contract (facts, availableRules in the view shape, planDiagnostics,
...
describe('evaluate() — composed output contract', () => {
  it('composes sheet + plan + offers for a legal attack -> smite turn', () => {
    ...
    // availableRules carries the view-shaped descriptor the UI reads.
  ...
  it('skips a planned action whose structural `when` gate is closed', () => {
    ...
    expect(out.status.legal).toBe(true); // inapplicable, not illegal
