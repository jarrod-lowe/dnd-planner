...
import { isGroupSettled, areDependenciesSatisfied, markRuleExecuted, markRuleSkipped, buildGroupStates, validateOrdering, validateCrossPhaseOrdering } from '$lib/rules-engine/ordering';
...
describe('areDependenciesSatisfied', () => {
	...
	it('returns true when all after groups are settled', () => {
		...
		const groups = new Map<string, GroupState>([
			['init', { name: 'init', phase: 'normal', ruleIds: ['a'], settled: true, executedRuleIds: [], skippedRuleIds: [] }]
		]);
	...
	it('returns false when an after group is not settled', () => {
		...
		const groups = new Map<string, GroupState>([
			['init', { name: 'init', phase: 'normal', ruleIds: ['a', 'b'], settled: false, executedRuleIds: ['a'], skippedRuleIds: [] }]
		]);
	...
	it('returns true when all multiple after groups are settled', () => {
		...
		const groups = new Map<string, GroupState>([
			['init', { name: 'init', phase: 'normal', ruleIds: [], settled: true, executedRuleIds: [], skippedRuleIds: [] }],
			['setup', { name: 'setup', phase: 'normal', ruleIds: [], settled: true, executedRuleIds: [], skippedRuleIds: [] }]
		]);
...
describe('markRuleExecuted', () => {
	it('adds rule id to executedRuleIds for each group the rule belongs to', () => {
		...
		const groups = new Map<string, GroupState>([
			['group-a', { name: 'group-a', phase: 'normal', ruleIds: ['rule-1'], settled: false, executedRuleIds: [], skippedRuleIds: [] }],
			['group-b', { name: 'group-b', phase: 'normal', ruleIds: ['rule-1'], settled: false, executedRuleIds: [], skippedRuleIds: [] }]
		]);
	...
	it('sets settled to true when all rules in group have been executed', () => {
		...
		const groups = new Map<string, GroupState>([
			['group-a', { name: 'group-a', phase: 'normal', ruleIds: ['rule-1', 'rule-2'], settled: false, executedRuleIds: ['rule-1'], skippedRuleIds: [] }]
		]);
	...
	it('does not set settled if some rules not yet processed', () => {
		...
		const groups = new Map<string, GroupState>([
			['group-a', { name: 'group-a', phase: 'normal', ruleIds: ['rule-1', 'rule-2'], settled: false, executedRuleIds: [], skippedRuleIds: [] }]
		]);
