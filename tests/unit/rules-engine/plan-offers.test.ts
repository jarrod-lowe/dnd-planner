 ...
 */
describe('plan reducer — ordered fold, no snapshots', () => {
  ...
  it('extra-attack: spends follow-up charges before actions, from real current state', () => {
    ...
    // No snapshot facts are needed or produced.
...
describe('offers — illegal-but-visible', () => {
...
describe('offers — unique ids (determinism)', () => {
...
describe('plan — legalWhen flags planned actions illegal (illegal-but-visible)', () => {
...
describe('plan — effect-based spends are visible within the turn (Codex plan.ts:94 fix)', () => {
...
describe('offers — structural when gate (omit vs. illegal-but-visible)', () => {
  ...
  it('applies a planned gated action while its gate is open, skips it once closed', () => {
    ...
    // planned action does not execute and spends nothing — the legacy engine skipped rules
