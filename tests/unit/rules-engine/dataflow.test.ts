...
import type { RuleModule } from '$lib/rules-engine';
import abilityScores from '$lib/rules-engine/rules/ability-scores';
import hp from '$lib/rules-engine/rules/hp';
import paladinLevel1 from '$lib/rules-engine/rules/class-paladin-level1';

/**
 * M0 spike, increment 1 — the dataflow "sheet" pass.
 *
 * This is the central proof that the engine kills *ordering fragility*. Previously,
 ...
 */
describe('dataflow sheet — ordering is structural, not authored', () => {
  ...
  it('settles ALL contributors to a fact before a dependent reads it (copy-after-settle, free)', () => {
    ...
    // The legacy engine needed the hp-total/hp-set/hp-copied three-group dance for this.
  ...
  it('reads an uncontributed fact as 0 ("facts start at zero")', () => {
  ...
  // statToModifierHandler returns 0 for an undefined (unset) score, so
